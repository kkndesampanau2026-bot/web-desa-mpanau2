...
import { useInfografisBansos } from '@/lib/queries'
import { api, ApiRequestError, type ApiSuccess } from '@/lib/api'
import { formatRupiah } from '@/lib/format'
import type { HasilCekBansos } from '@/types/api'
...
export function BansosPage() {
  const { data, isPending, error } = useInfografisBansos()
      ...
      {data && (
        <div className="mx-auto max-w-5xl px-6 py-10">
          <h1 className="font-heading text-2xl font-bold text-navy">
            Bantuan Sosial {data.tahun_anggaran}
          </h1>
            ...
            <section className="mt-10">
              ...
              <ul className="mt-3 space-y-3">
                {data.per_jenis.map((j) => (
                  <li key={j.jenis_bantuan} className="rounded-2xl border border-black/5 bg-white shadow-sm p-4">
 ...
 */
function FormCekPenerima() {
  const [nama, setNama] = useState('')
  const [digit, setDigit] = useState('')
  const [hasil, setHasil] = useState<HasilCekBansos | null>(null)
  const [galat, setGalat] = useState<ApiRequestError | null>(null)

  const cek = useMutation({
    mutationFn: async () => {
      const r = await api.post<ApiSuccess<HasilCekBansos>>('/infografis/bansos/cek', {
        nama,
    ...
    onError: (e) => {
      setHasil(null)
      setGalat(
        e instanceof ApiRequestError ? e : new ApiRequestError('Terjadi kesalahan.', 0)
      )
    },
  })
  ...
  return (
    <section className="mt-12 rounded-2xl border border-black/5 bg-navy/3 p-6">
      ...
      <p className="mt-1 text-sm text-slate-600">
        Periksa apakah Anda terdaftar sebagai penerima bantuan sosial desa. Untuk
        menjaga kerahasiaan data warga, cukup masukkan nama lengkap dan{' '}
        <strong>empat digit terakhir</strong> NIK Anda — jangan memasukkan NIK
        secara lengkap.
      </p>
      ...
      <form onSubmit={kirim} className="mt-5 grid gap-4 sm:grid-cols-[1fr_auto_auto] sm:items-end">
        <div>
          ...
          />
          {galat?.fieldError('nama') && (
            <p className="mt-1 text-sm text-red-600">{galat.fieldError('nama')}</p>
          )}
        </div>
        ...
        <div>
          ...
          <input
            ...
            pattern="\d{4}"
            value={digit}
            onChange={(e) => setDigit(e.target.value.replace(/\D/g, '').slice(0, 4))}
            placeholder="1234"
          ...
          />
          {galat?.fieldError('empat_digit_nik') && (
            <p className="mt-1 text-sm text-red-600">{galat.fieldError('empat_digit_nik')}</p>
          )}
        </div>

        <button
          type="submit"
          disabled={cek.isPending}
          className="rounded-lg bg-navy px-5 py-2 text-sm font-semibold text-white hover:bg-navy-light disabled:opacity-60"
        >
          {cek.isPending ? 'Mencari…' : 'Cek'}
        </button>
      </form>

      {/* Galat non-validasi — terutama pesan pembatasan laju (429). */}
      {galat && !galat.errors && (
