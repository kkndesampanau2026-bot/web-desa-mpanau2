...
import { useBerita, usePengaturan, useProfil } from '@/lib/queries'
...
export function BerandaPage() {
  ...
  const { data: berita } = useBerita({ page: 1 })
      ...
      <IsiHalaman lebar="lebar">
        <section aria-labelledby="jelajahi">
          ...
          <ul className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {PINTASAN.map((p) => (
              <li key={p.ke}>
                <Link to={p.ke} className="group block h-full">
        ...
        {berita && berita.items.length > 0 && (
          <section className="mt-14" aria-labelledby="berita-terbaru">
            ...
            <ul className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {berita.items.slice(0, 3).map((item) => (
                <li key={item.id}>
                  <Link to={`/berita/${item.slug}`} className="group block h-full">
