...
### Shared CMS elements (`resources/js/Components/Admin/`)

Admin screens don't hand-roll these — all three are already mounted or exported:

- `Form.tsx` — inputs, `Kartu`, `Pilihan`, and the action primitives. Row-level actions are **icons, never text links**: `TombolIkon` / `TautanIkon` (one icon + a required `label` that becomes the `aria-label`, plus an optional shorter `judul` tooltip), and `AksiBaris` for the usual Sunting/Hapus pair — it also takes `pesanHapus` for a custom confirmation message and `anak` for extra per-row icons (unduh, buka di situs publik) so they share one icon spacing. Buttons that carry a label (`Tombol`: Simpan, Tambah, Batal) stay text. `useGulirKeForm()` scrolls the form into view when a row's edit icon is pressed — every CMS screen with an edit flow uses it, and nothing calls `window.scrollTo` any more (scrolling to the page top only happened to be right while the form was the first element). Two shapes: a form that sits **above** its list takes `wadahRef={formulir.ref}` on its `Kartu` and calls `gulir()` inside the edit handler; a form that **replaces** its list (Berita, penerima bansos, tanggapan PPID, detail pengaduan) puts the ref on its own `<form>` and calls `gulir()` from a mount effect instead — in that shape the edit handler is in the parent, which has nothing left on screen to scroll to. `gulir()` waits one frame before scrolling, so it also works on forms that only appear on the same click (`{formTerbuka && <Kartu …>}`).
- `Dialog.tsx` — `useKonfirmasi()` for destructive confirmations. **Never `window.confirm`**: a browser that has been told "prevent this page from creating more dialogs" silently answers *cancel* to every later confirm, and the Hapus button just looks broken.
- `Toast.tsx` — `tampilkanToast(pesan, jenis?)` for save/delete outcomes, top-centre of the screen. Its container lives in `LayoutAdmin`, so pages only call the function. Every admin save/delete says *something*: react-query screens call it from `onSuccess`, while the Inertia screens (Akun Saya, Banner, Akun Operator) need no call at all — `LayoutAdmin` subscribes to Inertia's `success` event and toasts `flash.sukses` itself. That listener is deliberately an event subscription rather than a `useEffect` on `flash.sukses`: two saves in a row produce the identical message, and an effect keyed on the value would not fire the second time. Per-field validation errors stay inline under the field; only whole-action outcomes become toasts.
...
6. **Public pages never roll their own page shell.** Layout comes from three primitives in `resources/js/Components/ui/index.tsx` — `GAYA_WADAH` (the one container width + responsive gutters, shared by navbar/content/footer so their left edges line up), `KepalaHalaman` (the navy page header every public page starts with), and `IsiHalaman` (`sempit` | `sedang` | `lebar`, centred inside that container — and whatever width a page picks, its `KepalaHalaman` must be given the same `lebar`, or the title and the content end up on two different left edges). A hand-written `mx-auto max-w-Nxl px-6` on a page is the thing this replaced — see `docs/DEVIASI.md` §A6. The site header is the only sticky element — module sub-navs (`BilahTab`) scroll with the content on purpose, so a second bar never stacks under the navbar; `--tinggi-header` in `app.css` exists for anchor `scroll-margin-top`, and no layout should hard-code a `top-*` in its place.
